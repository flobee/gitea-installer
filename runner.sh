...
if [ "${ACTION_ASKQUESTIONS}" = "Y" ]; then
    ...
    #confirmCommand "${ACTION_BACKUPDEFAULT}";
    if confirmCommand "${ACTION_BACKUPDEFAULT}" && [ "${CONFIRMCOMMAND}" = "Y" ]; then
...
if [ "${ACTION_ASKQUESTIONS}" = "Y" ]; then
    ...
    read -r -p "install (I) or update (U) (def:'${_ACTION_TYPE}'): " ACTION_TYPE;
...
if [ "${ACTION_TYPE}" != 'I' ] && [ "${ACTION_TYPE}" != 'U' ]; then
    echo "ACTION_TYPE '$ACTION_TYPE' invalid, check config, exit";
    exit 1;
fi
