...
if [ ! -x "$(which "${PKG_MANAGER}")" ]; then
    ...
    echo '+ ---------------------------------------------------------------------';
    echo '| Please check this script or make the following packages available ';
    echo '| for your OS:';
...
for PACKAGE in ${PACKAGES_REQUIRED}; do
    if [ ! "$(${CMD_PKG_MANAGER_CHECK} "${PACKAGE}")" ]; then
        ...
        echo "Package check: '$PACKAGE' exists.";
    fi
done;
...
for PACKAGE in ${PACKAGES_OPTIONAL}; do
    if [ ! "$(${CMD_PKG_MANAGER_CHECK} "${PACKAGE}")" ]; then
        ...
        echo "Optional package check: '$PACKAGE' exists.";
    fi
done;
...
if [ "${PACKAGES_REQUIRED_MISSING}" != "" ] || [ "${PACKAGES_OPTIONAL_MISSING}" != "" ]; then
    ...
    echo '+ ---------------------------------------------------------------------';
     ...
     echo "No missing reqired package or optional package to be installed found.";
     echo
fi
...
if [ "${PACKAGES_STOP_ON_INCOMPLETE}" = "Y" ] && [ "${PACKAGES_REQUIRED_MISSING}" != "" ]; then
    ...
    echo "first. Run install.sh/ runner.sh after it again.";
    echo '+ ---------------------------------------------------------------------';
    exit 1;
fi
